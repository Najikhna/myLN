...
// BENTUK IKON (Disimpan di JS biar langsung ganti tanpa error)
...
// 1. SAAT WEB PERTAMA KALI DIBUKA
document.addEventListener('DOMContentLoaded', () => {
    // Set Tema Terakhir (Kalau belum ada, otomatis Gelap)
    ...
    // Tarik Data Blogger Langsung (Karena JS error kemarin, ini jadi gagal)
    fetchChapters();
});

// 2. FUNGSI MUNCULKAN / TUTUP LOGIN
...
// 3. FITUR KLIK MATA PASSWORD
function togglePasswordVisibility() {
    const passInput = document.getElementById('password');
    const eyeBtn = document.getElementById('toggle-password'); // Memanggil kotak tombol matanya
    
    if (passInput.type === 'password') {
        passInput.type = 'text';
        eyeBtn.innerHTML = svgEyeClosed; // Berubah jadi mata tertutup
    } else {
        passInput.type = 'password';
        eyeBtn.innerHTML = svgEyeOpen; // Berubah jadi mata terbuka
    }
}

// 4. FITUR TEMA DAN ANIMASINYA
function toggleTheme() {
    const body = document.body;
    
    // Memicu animasi muter berkali-kali
    themeBtn.classList.remove('spin-anim');
    void themeBtn.offsetWidth; // Teknik khusus agar animasi ter-reset
...
loginForm.addEventListener('submit', (e) => {
    ...
    if (u === 'admin' && p === 'adminsukamommy') { 
        processLogin(u); 
    } else if (u === 'guest' && p === 'guest-guest') { 
        processLogin(u); 
    } else { 
        errorElement.innerText = "Username atau Sandi salah!"; 
    }
});
...
async function fetchChapters() {
    try {
        ...
        document.getElementById('loading-msg').innerText = "Gagal memuat data. Cek koneksi internet.";
    }
}

// 7. TAMPILKAN KOTAK CHAPTER
